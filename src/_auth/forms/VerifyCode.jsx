...
const VerifyCode = () => {
  ...
  {/*white side */}
    <div className='justify-center items-center mx-auto mt-32'>
      ...
      <div>
        <h1 className='text-blue-600 font-bold text-3xl py-2'>Verify Code</h1>
        <p className='font-normal text-base text-black'>An authentication code has been sent to your email.</p>
      </div>
      <div>
       {/* form */}
       <Formik
...
// })
  ...
  >
    {({ errors, touched }) => (
      <Form className='mt-8' >
        <fieldset>
          <label className='font-bold text-xl' htmlFor="code">Enter code</label>
          <div className='relative flex flex-col gap-1 ' >
          <Field id='code' className='text-sm font-normal text-black border-2 rounded-md py-2 px-4 border-[#0540F2]' name="code" type={toggle ?'text':'code'}
          ...
          <p className='font-normal text-base'>Didn’t receive a code?<button className='text-red-500 font-semibold'>Resend</button></p>
       </fieldset>
       {errors.code && touched.code ? <div className=''>{errors.code}</div> : null}
       <button className='w-full mt-10 border-2 border-blue-600  py-2 bg-blue-600 rounded-full text-base font-semibold text-white' 
        onClick={() => navigate('/login')} type="submit">Verify</button>
      </Form>
    )}
  </Formik>
      </div>
